...
A requisição da malha também foi fixada em `outputs/tables/proveniencia_malha_municipal.csv`. A API v3 de malhas não expõe a edição no endereço; por isso, a geometria usada no estudo está preservada em `data/reference/malha_municipios_rs_2025_intermediaria.geojson`, identificada por SHA-256 e validada em 4 de setembro de 2026 contra a **Malha Municipal Digital 2025** do IBGE. O pacote oficial do RS contém 499 feições: 497 municípios e duas áreas operacionais (`Lagoa Mirim` e `Lagoa dos Patos`), excluídas da análise por não serem municípios. As duas fontes produziram exatamente os mesmos 1.400 pares de vizinhos na matriz de contiguidade Queen. A reexecução preservou os cinco índices de Moran Global e todas as conclusões de significância.
